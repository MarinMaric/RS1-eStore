...
namespace eStore_web.Controllers
{
    public class AdministratorController : Controller
    {
        ...
        public IActionResult OdobriSet(int id)
        {
            ...
            db.Update(igra);
            db.SaveChanges();
            _notifikacije.posaljiNotifikacije(igra.Developer.DeveloperID, (int)Autentifikacija.GetLogiraniKorisnik(HttpContext).BaseID, new NotifikacijaVM()
            {
        ...
        public IActionResult PregledIgara(int DeveloperID)
        {
            var model = new PregledIgaraAdministratorVM
            {
                ...
                Developer = db.Developer.Where(x => x.DeveloperID == DeveloperID).SingleOrDefault().NazivKompanije
            };
        ...
        public IActionResult Detalji(int DeveloperID)
        {
            ...
            var developer = db.Developer.Where(x => x.DeveloperID == DeveloperID).Include(x=>x.AccountBase.EmailAddress).SingleOrDefault();
            var model = new AdministratorDetaljiVM
            {
                DeveloperID=developer.DeveloperID,
                Email = developer.AccountBase.EmailAddress.Email,
                DatumUtemeljenja = developer.DatumUtemeljenja,
                Kompanija = developer.NazivKompanije,
                Grad = developer.LokacijaAdresa
            };
            return View(model);
        }
        ...
        {
            var lista = db.Refund.Include(x => x.Kupac).Include(x => x.Kupac.AccountBase).Include(x => x.Igra).ToList();
            return View(lista);
        }
        ...
        public IActionResult PregledReporta()
        {
            ...
            {
                Id = x.Id,
                ReportovaniKupac = x.ReportovaniKupac.Ime + " " + x.ReportovaniKupac.Prezime,
