...
namespace eStore_web.Common
{
    ...
    public class MyAuthorizeImpl : IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext filterContext, ActionExecutionDelegate next)
        {
            ...
            //kupci
            if (_Kupac == true && db.Kupac.Where(s => s.BaseID == k.BaseID).FirstOrDefault() != null)
            {
                await next(); //ok - ima pravo pristupa
                return;
            }
            ...
            //developeri
            if (_Developer && db.Developer.Where(s => s.BaseID == k.BaseID).FirstOrDefault() != null)
            {
                await next();//ok - ima pravo pristupa
                return;
            }
            if (_Administrator && db.Administrator.Where(s => s.BaseID == k.BaseID).FirstOrDefault() != null)
            {
                await next();//ok - ima pravo pristupa
                return;
            }
