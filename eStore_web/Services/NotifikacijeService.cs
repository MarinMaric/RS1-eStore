...
namespace eStore_web.Services
{
    public class NotifikacijeService : INotifikacije
    {
        ...
        public Task getAllNotifikacije(int br, string connectionId)
        {
            ...
            if (userTip == 3)
            {
                notifikacije = _db.Notifikacija.Include(x=>x.UserFrom).ThenInclude(y=>y.LoginInfo).OrderBy(x => x.Vrijeme).Where(x => x.UserTo == null).Select(x => new NotifikacijaVM()
                {
                    NotifikacijaId = x.NotifikacijaID,
                    Poruka = x.Poruka,
                    User = x.UserFrom.LoginInfo.AccountName,
            ...
            {
                notifikacije = _db.Notifikacija.Include(x => x.UserFrom).ThenInclude(y => y.LoginInfo).OrderBy(x => x.Vrijeme).Where(x => x.UserTo.LoginInfo.SignalRToken == connectionId).Select(x => new NotifikacijaVM()
                {
                    NotifikacijaId = x.NotifikacijaID,
                    Poruka = x.Poruka,
                    User = x.UserFrom.LoginInfo.AccountName,
        ...
        public async void posaljiNotifikacije(int to, int from, NotifikacijaVM message)
        {
            ...
            message.User = userFrom.AccountName;
