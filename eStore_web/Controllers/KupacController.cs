    ...
    public class KupacController : Controller
    {
        ...
        public IActionResult GameDetailes(int igraID)
        {
            ...
            if (LoginInfo != null)
            {
                ...
                {
                    Kupac Kupac = db.Kupac.Where(k => k.BaseID == LoginInfo.BaseID).FirstOrDefault();
                    if (Kupac != null)
                    {
            ...
            Igra igra = db.Igra.Where(i => i.IgraID == igraID)
                .FirstOrDefault();
            Popust popust = db.Popust.Where(p => p.IgraID == igraID && p.Aktivan==true && p.PocetakPopusta <= DateTime.Now && p.KrajPopusta>=DateTime.Now).FirstOrDefault();
        ...
        public IActionResult KupiIgru(int KupacID,int IgraID)
        {
            ...
            if (cijena < Kupac.Wallet.balance)
            {
                ...
                WalletHistory walletHistory = new WalletHistory()
                {
                    ...
                    IsIgra = true,
                    TransactionAmount = (double)cijena * (-1),
                    CurrentBalance = Kupac.Wallet.balance - (cijena),
                    IgraID=IgraID
                };
